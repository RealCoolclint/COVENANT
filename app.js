...
window.addEventListener('load', () => {
  const splash = document.getElementById('covenant-splash');
  const flash = document.getElementById('covenant-flash');
  const patch = document.getElementById('covenant-splash-patch');
  const versionEl = document.getElementById('covenant-splash-version');
  const grid = document.getElementById('covenant-splash-grid');

  const now = new Date();
  const jj = String(now.getDate()).padStart(2, '0');
  const mm = String(now.getMonth() + 1).padStart(2, '0');
  const aa = now.getFullYear();
  if (versionEl) versionEl.textContent = 'V1.' + jj + '.' + mm + '.' + aa;
